...
export function CartDrawer({
  ...
  const [bulkWarnOpen, setBulkWarnOpen] =
    useState(false);
  ...
  return (
    <>
        ...
        {open && (
          <>
            ...
            >
              <div
                aria-hidden
                className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full bg-sakura/30 blur-3xl"
              />

              <div
                aria-hidden
                className="pointer-events-none absolute -left-20 top-1/2 h-56 w-56 rounded-full bg-petal/40 blur-3xl"
              />

              <div className="relative flex items-center justify-between border-b border-ink/10 px-6 py-6">
                <div>
                  <p className="text-[0.65rem] uppercase tracking-[0.35em] text-cherry">
                    Your basket
                  </p>
                  ...
                  <h2 className="mt-1 font-display text-3xl leading-none">
                    {count > 0 ? (
                      <>
                        {count}{" "}
                        <span className="italic text-cherry">
                          bites
                        </span>
                      </>
                    ) : (
                          ...
                          <div className="flex flex-1 flex-col">
                            ...
                            <div className="mt-auto flex items-center justify-between">
                              <div className="flex items-center gap-1 rounded-full border border-ink/10 bg-cream p-0.5 shadow-inner">
                                ...
                                <span className="w-7 text-center font-display text-base tabular-nums">
                                  {item.quantity}
                                </span>

                                <button
                                  type="button"
                                  className="flex h-7 w-7 items-center justify-center rounded-full text-ink transition hover:bg-ink hover:text-cream disabled:cursor-not-allowed disabled:opacity-30"
                                  onClick={() =>
                                    setQty(
                                      item.menu_item_id,
                                      item.quantity + 1,
              ...
              <motion.div
                layout
                className="relative border-t border-ink/10 bg-gradient-to-b from-blossom/90 to-cream px-6 py-5"
              >
                {isBulk && (
                  <div className="mb-3 flex items-start gap-2 rounded-2xl border border-cherry/30 bg-cherry/10 p-3 text-xs leading-5 text-ink/80">
                    <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-cherry" />
      ...
      >
        <DialogContent className="max-w-lg rounded-3xl border-cherry/20">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 font-display text-3xl text-cherry">
              <AlertTriangle className="h-7 w-7 shrink-0" />

              {copy.title}
            </DialogTitle>

            <DialogDescription className="pt-2 text-left text-sm leading-6 text-ink/70">
              {copy.body}
            </DialogDescription>
          </DialogHeader>

          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-ink/10 bg-petal/50 p-4">
              <p className="text-[0.65rem] font-bold uppercase tracking-[0.25em] text-ink/50">
                Normal kitchen
              </p>

              <p className="mt-2 font-display text-3xl tabular-nums">
                ~{copy.normalMinutes}m
              </p>
            ...
            <div className="rounded-2xl border border-cherry/30 bg-cherry/10 p-4">
              ...
              <p className="mt-2 font-display text-3xl tabular-nums text-cherry">
                ~{copy.bulkMinutes}m
              </p>
