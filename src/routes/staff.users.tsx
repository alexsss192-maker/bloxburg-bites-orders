        ...
        <div className="overflow-hidden rounded-3xl border border-border/60 bg-white shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-secondary text-left">
              <tr><th className="p-3">Email</th><th className="p-3">Admin</th><th className="p-3">Chef</th></tr>
            </thead>
            <tbody>
              {(data ?? []).map((u) => (
                <tr key={u.id} className="border-t border-border/60">
                  <td className="p-3 font-medium">{u.email || u.id}</td>
                  <td className="p-3"><input type="checkbox" checked={u.roles.includes("admin")} onChange={(e) => toggle.mutate({ user_id: u.id, role: "admin", enabled: e.target.checked })} /></td>
                  <td className="p-3"><input type="checkbox" checked={u.roles.includes("chef")} onChange={(e) => toggle.mutate({ user_id: u.id, role: "chef", enabled: e.target.checked })} /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <DialogContent className="max-w-md rounded-3xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); create.mutate(form); }} className="space-y-4">
            <div><Label>Email</Label>
              <Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required className="mt-2" /></div>
            <div><Label>Password (min 6 chars)</Label>
              <Input type="text" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required minLength={6} className="mt-2" /></div>
            <div><Label>Role</Label>
              <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as "admin" | "chef" })}
                ...
                <option value="chef">Chef</option><option value="admin">Admin</option>
              </select></div>
